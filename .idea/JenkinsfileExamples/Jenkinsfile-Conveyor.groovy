node {
    ...
    docker.image(env.DOCKER_REGISTRY + '/compozed/ci-base:0.6').inside {
        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'AVC-ADMIN', passwordVariable: 'CF_PASSWORD', usernameVariable: 'CF_USER'],
                         [$class: 'UsernamePasswordMultiBinding', credentialsId: 'SPRING-DATASOURCE-STAGING', passwordVariable: 'DATASOURCE_PASSWORD', usernameVariable: 'DATASOURCE_USER'],
                         [$class: 'UsernamePasswordMultiBinding', credentialsId: 'FLYWAY_STAGING', passwordVariable: 'FLYWAY_PASSWORD', usernameVariable: 'FLYWAY_USER'],
        ]) {
            Client: {
...
- name: boilerplate-client-staging
  ...
  domain: auth-platform-sandbox.allstate.com""",\
                      ...
                      serviceNowGroup: 'REPLACE_WITH_YOUR_CHANGE_GROUP', serviceNowUserID: "${BUILD_USER_ID}", space: 'STAGING', username: "${env.CF_USER}"])
            }
            Server: {
                // This calls the conveyor plugin
                step([$class: 'ConveyorJenkinsPlugin', applicationName: "boilerplate-server-staging",
